---
title: "Differences with SDMs"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Differcenes with SDMs}
...
Here, we present the differences between classic SDM and our trophic SDM, starting from the simplest case when these two approaches coincide.
...
We now assume that data on species distribution are continuous abundances (e.g. basal area, biomass), that we can model with a linear regression. We name $y_{iA}$ and $y_{iB}$ the observations of species $y_A$ and $y_B$ respectively and we assume that the environment have a quadratic effect on both species.
...
\end{aligned}
$$
We hereafter focus on the expectation of the models (corresponding to the predicted values), focusing in particular on species 𝑦$y_B$ for which the two models are different. For classic SDMs:  
$$E[y_{iB} | \ x_i] = \beta^{SDM}_{0B} + \beta^{SDM}_{1B} x_i + \beta^{SDM}_{2B}x_i^2 $$
For trophic SDM, we need to condition on a value of $y_{iA}$. We here choose to condition on the expected value of  $y_{iA}$, which corresponds to using predictions from $y_A$ to predict $y_B$ (as a technical note,we remark that in this linear linear model, with Gaussian data and a linear interaction between prey and predator, we have that $E_A[E_B[y_B | \ y_A]] = E_B[y_B | \ E[y_A]]$, which is not necessarily true in other cases).  
We thus have:  
...
```
As we showed above, the coefficient of classic SDM for species $y_B$ are equal to the environmental coefficients of trophic SDM for species $y_B$ plus the indirect effect of the environment on species $y_B$ (given by the product of the biotic term and the environmental coefficients of species $y_A$).
As a consequence, the predictions of both models coincide when the predicted value of species  $y_A$ is used to predict  $y_B$.
However, notice that our trophic model allows to retrieve the correct fundamental niche of species 𝑦𝐵
```{r fundamental}
par(mfrow = c(1,1))
...
```
We now compare regression coefficients and predictions from the two models
...
```
The relationship between the coefficients of the two models does not hold anymore more, and predictions are now different, with trophic SDM that are largely better than classic SDM for species 𝑦𝐵 (R2 of 0.83 and 0.96 respectively).

### Non-continous abundances
Let’s for example consider here the case of presence-absences (i.e. binary data), without modifying the other two conditions. Therefore, we use a (generalised) linear model like logistic regression, and we model a linear relationship (at the link level) between predator and prey. Thus, for SDM we have:  
$$
\begin{aligned}
...
Notice that the equality between the models does not hold anymore due to the non linearity of the logit function. Indeed:
$$
\begin{aligned}
...
#### Simulation
We now confirm this analytically derivation with a simple example. The code is very similar to the previous one, but we now generate binary data from a logistic model.
```{r simlogi}
...
```
We then fit classic SDM and a trophic SDM
```{r fitbinary, warning = F}
# For species A, the two models coincide
...
```
We now test the relationships between model coefficients and wheter the equality of model predictions still holds
...
```
The relationship between the coefficients of the two models does not hold anymore more, and predictions are now different, with trophic SDM that slightly better than classic SDM in term of AUC for species $y_B$.

### Non-linear statistical model
We finally show how using a non-linear statistical model leads to different predictions from classical SDMs and a trophic SDM with the example of a random forest. Since we cannot provide any analytical derivation for random forest, we simply provide an example.
Data are simulated exactly as in the very first examples.
```{r simRF}
# Simulate data for the basal species A 
A = 3*X + 2*X^2 + rnorm(100, sd = 5)
# Simulate data for the predator species B
...
```
We now compare regression the predictions from the two models.
```{r predictRF}
# Prediction are now different
