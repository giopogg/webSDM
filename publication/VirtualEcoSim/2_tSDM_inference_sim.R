...
# By running this script once, you analyse one 'replication' (i.e. 2550 communities) of the simulated datasets that were
# previously run wiht 1_Community_simulations.R
# To obtain *exactly* the results of the paper, this script should be run 100 times. For each run, the variable 'job'
# should be specified with a number from 1 to 100. This parameter tells which of the 100 replications should be analysed. 

# !!! Only the working directory and the parameter job should be specified to run the script

# By running this script, it will enrich the folder created by 1_Community_simulations.R with the related job.
# It will in particular create a folder 'Fig', with subdirectory for every theoretical model, containing all the necessary figures
############################################################
...
source("2_Tools_tSDM_functions.R") # Script for running functions. The documentation could be outdated, please refer to the documentation of the R package.
  
#################################################################################################
#### Test on simulated data
################################################################################################

# !!!!!!! Specify which replicates should be analysed
job = 1
...
strengthBI = as.numeric(gsub(".*maxBI|_.*", "", simPath)) # strength of biotic interactions
...
for (i in 1:length(SIMlist)){
  ...
  post_j = as.data.frame(posterior_summary(model_j))
            estimates_stan=rbind(estimates_stan,
                                 data.frame(p.est=if(linear)post_j$Estimate else c(post_j$Estimate[1], (0:100/100)[which.max(post_j$Estimate[3]*(0:100/100)**2 + post_j$Estimate[2]*0:100/100 + post_j$Estimate[1])], post_j$Estimate[grepl("Y", row.names(post_j))]),
...
for (i in 1:length(SIMlist)){
  ...
  for (j in 1:S){
    ...
    post_j = as.data.frame(posterior_summary(model_j))
    SDM.estimates_stan=rbind(SDM.estimates_stan,
                             data.frame(p.est=if(linear)post_j$Estimate else c(post_j$Estimate[1], (0:100/100)[which.max(post_j$Estimate[3]*(0:100/100)**2 + post_j$Estimate[2]*0:100/100 + post_j$Estimate[1])], post_j$Estimate[grepl("Y", row.names(post_j))]),
                                         est.02=if(linear)post_j$Q2.5 else NA,
                                         est.97=if(linear)post_j$Q97.5 else NA,
