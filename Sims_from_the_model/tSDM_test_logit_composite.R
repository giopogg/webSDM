...
colnames(A)=rownames(A)=paste0("Y_",as.character(1:S))
...
A_lin=A_quad=vector()
for(j in 1:S){
  if(length(neighbors(G,colnames(A)[j],mode=c("out")))>0){
  ...
  for(j in 1:S){
    ...
    if(simul_type=="true_logit"){
      ...
      if(length(neighbors(G,colnames(Y)[j],mode=c("out")))>0){
        ...
        V[,j]=X%*%B[j,]+ A_lin[j]*rich + A_quad[j]*(rich^2)
      }else{
        V[,j]=X%*%B[j,]
      }
      prob[,j]=1/(1+exp(-V[,j]))
    ...
    if(simul_type=="logit_threshold"){
      ...
      if(length(neighbors(G,colnames(Y)[j],mode=c("out")))>0){
        preys= Y[,neighbors(G,colnames(Y)[j],mode=c("out"))]
        if(is.null(ncol(preys))){rich=preys}else{rich=rowSums(preys)}
        V[,j]=X%*%B[j,]+ A_lin[j]*rich + A_quad[j]*(rich^2)+rnorm(n+n_oos,0,sqrt(sigma_2[j]))
      }else{
        V[,j]=X%*%B[j,]+rnorm(n+n_oos,0,sqrt(sigma_2[j]))
      }
        prob[,j]=1/(1+exp(-V[,j]))
    ...
    if(simul_type=="logit_prob_covariate"){
      if(length(neighbors(G,colnames(Y)[j],mode=c("out")))>0){
        preys= prob[,neighbors(G,colnames(Y)[j],mode=c("out"))]
        if(is.null(ncol(preys))){rich=preys}else{rich=rowSums(preys)}
        V[,j]=X%*%B[j,]+ A_lin[j]*rich + A_quad[j]*(rich^2)
      }else{
        V[,j]=X%*%B[j,]
...
# fit the models
m_glm = trophicSDM(Y=Y,X=X_raw,G=G,env.formula=env.form,penal=NULL,sp.formula=sp.formula,sp.partition=NULL,method="glm",family=binomial(link = "logit"),iter=1000)
...
m_stan = trophicSDM(Y=Y,X=X_raw,G=G,env.formula=env.form,penal=NULL,sp.formula=sp.formula,sp.partition=NULL,method="stan_glm",family=binomial(link = "logit"),iter=1000)
...
m_bayes = trophicSDM(Y=Y,X=X_raw,G=G,env.formula=env.form,penal=NULL,sp.formula=sp.formula,sp.partition=NULL,method="bayesglm",family=binomial(link = "logit"),iter=1000)
