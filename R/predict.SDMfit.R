...
#' @param prob.cov Only for preence-absence data. If set to FALSE, it gives back also predicted presence-absences (which is then used by trophicSDM.predict to predict the predators).
#' @param ... 	additional arguments
#' @return A list containing for each species the predicted value at each sites. If method = "stan_glm", then each element of the list is a sites x pred_samples matrix containing the posterior predictive distribution of the species at each sites. If prob.cov = TRUE, it returns a list contaning:
#' \itemize{
...
#' # the predicted occurrences of its preys. Here we simply give, as an example
#' # the observed distribution of the preys. This function is sequantially 
#' # called in the prediction of trophicSDM, so that preys are predicted as a function
#' # of the environment and so on.
#' predict(m$model$Y1, newdata = cbind(X, Y))
